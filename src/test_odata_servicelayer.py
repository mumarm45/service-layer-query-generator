...
from modules.hybrid_retriever import create_hybrid_retriever, hybrid_retrieve
from datetime import date
import os
root_path = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))

# Cache for expensive objects (built once, reused)
# Clear cache by restarting the app if prompt changes
_cache = {}

def _get_cached_resources():
    """Load and cache hybrid retriever, LLM, and chain (only built once)."""
    if not _cache:
        ...
        # Use hybrid retriever (BM25 + FAISS) for better search
        _cache['hybrid_retriever'] = create_hybrid_retriever(chunks, embedding_model)
...
def perform_odata_query(query):
    ...
    if resources['hybrid_retriever']:
        ...
        relevant_docs = hybrid_retrieve(query, resources['hybrid_retriever'])
